...
class prgressBarchange(QThread):
    ...
    def run(self):
        ...
        while count < limit:
            ...
            # print(count)            
            count +=1
            remain= int((1- ((limit-count)/limit)) * 100)
            print(remain)
            self.changeVal.emit(count) # return 값
...
class Exe_Dialog(QDialog): # 이부분은 object를 QDialog로 변경
    ...
    def processing(self, val):
        self.exeProgressBar.setMaximum(100)
        self.exeProgressBar.setValue(val)       
     

    def progressValchange(self):
        self.progressValthread = prgressBarchange(myvar = 100)                
        self.progressValthread.changeVal.connect(self.processing)
        self.progressValthread.start()

    def closeEvent(self, event):        
        self.progressValthread.terminate()         
